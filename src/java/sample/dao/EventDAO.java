...
public class EventDAO {
    ...
    private static final String SEARCH_NAME_EVENT ="SELECT eventID, categoryName, locationName, eventName, eventDetail, image, FORMAT(startTime, 'yyyy-MM-dd') AS startTime, FORMAT(endTime, 'yyyy-MM-dd') AS endTime, numberOfAttendees, formality, ticketPrice FROM tblEvent e, tblLocation l, tblCategory c WHERE e.locationID = l.locationID AND e.categoryID = c.categoryID AND e.eventName LIKE ? AND status = '1'";
    ...
    public List<EventDTO> getListEvent(String searchEvent) throws SQLException {
        ...
        try {
            conn = DBUtils.getConnection();
            if (conn != null) {
                ptm = conn.prepareStatement(SEARCH_NAME_EVENT);
                ptm.setString(1, "%" + searchEvent + "%");
                rs = ptm.executeQuery();
                while (rs.next()) {
                    //String eventID = rs.getString("eventID");
                    String categoryName = rs.getString("categoryName");
